...
{#if show}
  ...
  <div>
    ...
    <div class="select-div">
      ...
      <select
        disabled={settings.classicChordOrder}
        title={settings.classicChordOrder
          ? 'Disable "Classic chord order" to customize this.'
          : ""}
        name="shifts-position"
        id="shifts-position"
        bind:value={settings.pShifts}
      >
    ...
    {#if hasMIDI}
      ...
      {#if settings.breaks == "manual"}
        <div class="beats">
          <label class="slider-label" for="beats-for-newline"
            >Break lines:
          </label>
          <input
            type="range"
            id="beats-for-newline"
            min="1"
            max="32"
            bind:value={settings.beats}
          />
          <span
            >Every {settings.beats == 1
              ? "1 beat"
              : `${settings.beats} beats`}</span
          >
        </div>
      {/if}
    {:else}
      <i
        >Some settings are not available because the original MIDI data is
        missing.</i
      >
    {/if}
    ...
    <label for="classic-chord-order">
      <input
        ...
        bind:checked={settings.classicChordOrder}
      />
      Classic chord order
    </label>
    ...
    <label for="order-quantizes">
      <input
        type="checkbox"
        id="order-quantizes"
        bind:checked={settings.sequentialQuantize}
      />
      <span
        class="cursor-help"
        title="Whether quantized chords should follow their original sequential order, or use regular chord sorting"
      >
    ...
    <label for="curly-quantizes">
      <input
        type="checkbox"
        id="curly-quantizes"
        bind:checked={settings.curlyQuantizes}
      />
    ...
    <label for="tempo-checkbox">
      <input
        type="checkbox"
        id="tempo-checkbox"
        bind:checked={settings.tempoMarks}
      />
    ...
    <label for="oormark-checkbox">
      <input
        type="checkbox"
        id="oormark-checkbox"
        bind:checked={settings.oorMarks}
      />
    ...
    {#if settings.oors && settings.oorMarks}
      <div>
        <label
          title="Helps tell you if notes are out-of-range, certain characters are restricted from use!"
          for="oor-separator">Out-of-range separator (?):</label
        >
    ...
    {#if hasMIDI && !settings.missingTempo}
      <label for="bpm-changes">
        <input
          type="checkbox"
          id="bpm-changes"
          bind:checked={settings.bpmChanges}
        />
      ...
      {#if settings.bpmChanges}
        ...
        <div class="beats">
          <label class="slider-label" for="min-speed-change"
            >Min. % speed change:
          </label>
          <input
            type="range"
            ...
            min="0"
            max="100"
            bind:value={settings.minSpeedChange}
          />
    ...
    >
      <div class="flex flex-row mt-3">
        <label class="flex flex-row items-center" for="line-height"
          >Line height:</label
        >
        <input
          class="w-32"
