...
<div class="flex flex-row gap-1 sticky top-0 z-10 app-background-color">
    ...
    <button disabled={settings.capturingImage} on:click={() => dispatch("captureSheetAsImage", {mode: "download", selectionOnly: hasSelection})}>
    ...
    {#if typeof ClipboardItem !== "undefined"}
        ...
        <button disabled={settings.capturingImage} on:click={() => dispatch("captureSheetAsImage", {mode: "copy", selectionOnly: hasSelection})}>
            {#if settings.capturingImage}
                Please Wait...
            {:else}
                Copy Image
