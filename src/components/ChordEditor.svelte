<script>
  ...
  let hasOverrides = () => {
    return !(
      [undefined, "keep"].includes(overrides.shifts) &&
  ...
  let updateWithOverrides = () => {
    ...
    let newChord = new Chord(
      newNotes,
      false,
      overrides.sequential ?? settings.sequentialQuantize,
    );
    if (hasOverrides()) {
...
<dialog
  bind:this={dialog}
  on:close={() => {
    overrides = { shifts: "keep", oors: "keep", sequential: undefined };
    dispatch("closed");
  }}
  on:click|self={() => {
    dialog.close();
...
>
  <div class="flex flex-col items-center gap-2">
    <div
      id="chord"
      style="background-color: #2D2A32"
      class="text-2xl p-1 rounded-md"
    >
          ...
          <div class="flex flex-row gap-2">
            <div class="select-div">
              ...
              <select
                ...
                id="shifts-position"
                bind:value={overrides.shifts}
                on:change={() => {
                  if (!("overrides" in tempBuffer)) tempBuffer.overrides = {};
        ...
        {#if tempBuffer.is_quantized && tempBuffer.notes.length > 1}
          <div class="flex flex-row gap-2">
            ...
            <input
              type="checkbox"
              id="sequential"
              bind:checked={overrides.sequential}
              on:change={() => {
                if (!("overrides" in tempBuffer)) tempBuffer.overrides = {};
