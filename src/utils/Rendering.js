...
function render_chord(chord, next, settings, selected) {
  ...
  const selection_color = settings?.capturingImage
    ? "rgba(0, 0, 0, 0)"
    : "rgba(97, 97, 97, 50)";
  ...
  let isChord =
    chord.notes.length > 1 && chord.notes.find((note) => note.valid === true);
  ...
  for (const note of chord.notes) {
    ...
    if (draw_as_oor === true) {
      ...
      let startOors = chord.notes.filter(
        (note) =>
          note.outOfRange === true && note.displayValue === note.value - 1024,
      );
      let endOors = chord.notes.filter(
        (note) =>
          note.outOfRange === true && note.displayValue === note.value + 1024,
      );
      ...
      const isChordWithOnlyEndOorsAndIsFirstEndOor =
        isChord &&
        nonOors.length === 0 &&
        startOors.length === 0 &&
        note === endOors[0];
      const isChordWithMoreThanOneNonOorAndIsFirstEndOor =
        isChord && nonOors.length > 0 && note === endOors[0];
      ...
      res +=
        `<span style="display:inline-flex; justify-content: center; min-width: 0.6em; border-bottom: 2px solid; font-weight: 900">` +
        `${text_representation}</span>`;

      if (settings.oorMarks)
        if (isLastStartOor && nonOors.length > 0) res += `'`;
