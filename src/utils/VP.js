...
let is_chord = (x) => {
  try {
    return "notes" in x && x.notes.length != 0;
  } catch {
    return false;
  }
};
...
}; // !x || (x.type && (x.type == "break" || x.type == "comment"))
class Note {
  constructor(
    value,
    playTime,
    ...
    if (typeof value == "object")
      // Copy constructor
      var { original, value, playTime, tempo, BPM, delta, shifts, oors } =
        value;
...
class Chord {
  constructor(
    ...
    if ("notes" in notes)
      // It's actually another chord - copy constructor
  ...
  transpose = (by, relative = false, mutate = false, skipOrdering = false) => {
    if (relative) by = this.transposition + by;

    let new_chord = new Chord(
      this.notes,
      this.classicChordOrder,
      this.sequentialQuantize,
    );
    let assignee = mutate ? this : new_chord;

    let new_notes = assignee.notes.map((note) =>
      Note.new_with_saved_original(note, note.original + by, skipOrdering),
    ); // create a note with the correct "original" value
    new_chord = new Chord(
      new_notes,
      this.classicChordOrder,
      this.sequentialQuantize,
  ...
  #sortChord(notes, classicChordOrder) {
    return classicChordOrder
      ? this.#classicChordOrderSort(notes)
      : notes.sort((a, b) => a.displayValue - b.displayValue);
  }
...
function BPMComment(previousBPM, newBPM, type, minimum) /*  */ {
  ...
  switch (type) {
    case "simple":
      ...
      for (let i = 0; i < increments; i++) text += char;

      if (text.length > 20) {
        // would be spam, not worth
...
function generateChords(
  events /* note_on, set_tempo and time_signature */,
  settings,
  previous_chords = undefined,
  ticks_per_beat = 480,
) {
  ...
  let set_time_signature = (event) => {
    ...
    current_time_signature = {
      ...
      get tick_resolution() {
        // only used for converting from playTime into ticks
  ...
  let debug_comment =
    window.debug_timing !== undefined
      ? (text) => {
          chords.push({ type: "comment", kind: "inline", text });
        }
      : () => {};
  ...
  function break_realistically(note, index = undefined) {
    if (previous_chords && index && !previously_reflowed.includes(index)) {
      if (
        previous_chords?.[index_of_index(previous_chords, index)]?.reflow ===
        true
      ) {
    ...
    if (!next_break) {
      ...
      if (next_break !== 0) next_break = current_ticks;
  ...
  const possibly_transpose_to_previous = (chord, index) => {
    ...
    if (chord_previously?.reflow === true) {
      chord.reflow = true;
    }

    if (
      chord_previously &&
      ![0, undefined].includes(chord_previously.transposition)
    ) {
  ...
  events.forEach((element) => {
    if (element.type == MIDIEvents.EVENT_META) {
      switch (element.subtype) {
        case MIDIEvents.EVENT_META_TIME_SIGNATURE:
          ...
          debug_comment(
            `<br>(switch to ${current_time_signature.numerator}/${2 ** current_time_signature.denominator})<br>`,
          );
          return;
        ...
        case MIDIEvents.EVENT_META_SET_TEMPO:
          ...
          debug_comment(
            ` <new tempo: ${current_time_signature.numerator}/${2 ** current_time_signature.denominator}>`,
          );
          ...
          if (previousTempo == undefined) {
            if (settings.bpmChanges)
              chords.push({
                type: "comment",
                kind: "tempo",
                text: `Tempo: ${Math.round(element.tempoBPM)} BPM`,
              });
          } else if (
            previousTempo != undefined &&
            previousTempo != element.tempo &&
            previousBPM != element.tempoBPM
          ) {
            if (settings.bpmChanges) {
              let comment = BPMComment(
                Math.round(previousBPM),
                Math.round(element.tempoBPM),
                settings.bpmType,
                settings.minSpeedChange,
              );
    ...
    if (
      element.type != MIDIEvents.EVENT_MIDI &&
      element.subtype != MIDIEvents.EVENT_MIDI_NOTE_ON
    )
      return;
    ...
    if (Math.abs(playtime - lastPlaytime) < quantize) {
      current_notes.push(
        new Note(key, playtime, ticks, nextTempo, nextBPM, delta, shifts, oors),
      );
 ...
 */
function best_transposition_for_chord(
  chord,
  deviation,
  stickTo = 0,
  resilience = 0,
 ...
 */
function best_transposition_for_monochord(
  chords,
  deviation,
  stickTo = 0,
  resilience = 4,
  ...
  let bests = best_transposition_for_chord(
    monochord,
    deviation,
    stickTo,
    resilience,
