<script>
  ...
  let existingProject = {
    ...
    proceed: (force_decision = "prompt") => {
      ...
      if (decision == "load" || decision == "existing") {
        ...
        let old_style_project = !chords_and_otherwise.find((e) => is_chord(e))
          ?.notes;
  ...
  $: {
    if (fileInput)
      filename = basename(fileInput.files[0]?.name) ?? existingProject.name;
  }
  ...
  function softRegen() {
    ...
    for (let i = 0; i < chords_and_otherwise.length; i++) {
      ...
      if (is_chord(chord)) {
        ...
        if (chord.overrides)
          new_chord.overrides = JSON.parse(JSON.stringify(chord.overrides));
  ...
  try {
    ...
    settings.bpm = 120;
    settings.capturingImage = false; // doesn't make sense to save these
  } catch (e) {
    settings = undefined;
  }
  $: {
    ...
    if (
      ["pShifts", "pOors", "classicChordOrder", "sequentialQuantize"].some(
        changed,
      )
    )
      softRegen();
    ...
    if (selectionOnly && has_selection) {
      ...
      options.filter = (node) => {
        if (node.classList?.contains("sheet-item")) {
          let index = +node.dataset.index;
          return (
            (index >= start && index <= end) || index === extraTransposeIndex
          );
        }
        return true;
      };
...
>
  ...
  >
    <div>
        ...
        {#if has_selection}
          <div class="flex flex-row justify-around items-center gap-2">
            <button
              class="w-full block"
              on:click={() => {
                transposeRegion(selection.left, selection.right, 1, {
                  relative: true,
                });
                repopulateTransposeComments();
      ...
      <SheetOptions bind:settings show={sheetReady} hasMIDI={!!MIDIObject} />
    </div>
    <Guide />
    <hr class="mx-2 my-1" />
    <a
                ...
                {:else}
                  ...
                  >
                    {#if inner.kind === "transpose"}
                      {@const [transposeText, transposeIndex] =
                        inner.text.split(" #")}
